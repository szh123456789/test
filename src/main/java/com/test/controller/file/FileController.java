...
import com.test.service.tools.video.fileService;
import com.test.service.tools.video.untils.Result;
...
public class FileController {
    ...
    public Result check(String key){
        List<file> check = fileservice.check(key);
        if (check.size()!=0){
    ...
    private void merge(file file1) throws Exception{
        //分段和并开始
        log.info("分段和并开始");
        //获取路径
        ...
        Long shardTotal = file1.getShardTotal();
        File newFile = new File(basePath + path+"."+file1.getSuffix());
        //文件追加写入
